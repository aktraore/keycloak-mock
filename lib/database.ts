...
class MockDatabase {
    ...
    allUsers(): MockUser[] {
        return this.users.filter(
            (storedUser) => storedUser.profile.email !== MockDatabase.SERVICE_USER_EMAIL,
        );
    }
    ...
    createUser(options?: CreateMockUserOptions): MockUser {
        ...
        if (this.findUserByUsername(username)) {
            throw new DuplicateUserError(`A user with username ${username} already exists`);
        }

        if (this.findUserByEmail(email)) {
            throw new DuplicateUserError(`A user with email ${email} already exists`);
        }
        ...
        const profile: MockUserProfile = {
            ...
            username,
            enabled: isNil(finalizedOptions.enabled) ? true : finalizedOptions.enabled,
